...
namespace iris {
	...
	uint16_t encodeUint16LE(byte* buf) {
		return encodeUint16LE(buf[0], buf[1]);
	}
	uint32_t encodeUint32LE(byte* buf) {
	...
	void decodeUint32LE(uint32_t value, byte* storage) {
		typedef uint32_t Number;
		typedef byte Field;
	...
	void decodeUint16LE(uint16_t value, byte* storage) {
		typedef uint16_t Number;
		typedef byte Field;
		storage[0] = decodeField<Number, Field, 0>(value);
		storage[1] = decodeField<Number, Field, 1>(value);
	...
	void decodeInt32LE(int32_t value, byte* storage) {
		typedef int32_t Number;
		typedef byte Field;
		storage[0] = decodeField<Number, Field, 0>(value);
		storage[1] = decodeField<Number, Field, 1>(value);
	...
	void decodeInt16LE(int16_t value, byte* storage) {
		typedef int16_t Number;
		typedef byte Field;
		storage[0] = decodeField<Number, Field, 0>(value);
		storage[1] = decodeField<Number, Field, 1>(value);
