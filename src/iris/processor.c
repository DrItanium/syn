...
#define PROCESSOR_DATA USER_ENVIRONMENT_DATA + 0
#define GetProcessorData(theEnv) ((processor*) GetEnvironmentData(theEnv, PROCESSOR_DATA))
static void reclaimprocessor(void*);
void main() {
   void* theEnv;
...
void installprocessorloop(processor* proc) {
   ...
   offset = encodeprintstring(proc, offset, TrueRegister, "I am a cat");
...
void EnvUserFunctions(void* theEnv) {
    if (! AllocateEnvironmentData(theEnv, PROCESSOR_DATA, sizeof(processor),reclaimprocessor))
    {
        sysfatal("Error allocating environment data for processor: %r");
        exits("allocprocessorfailure");
    }
    setupprocessor(GetProcessorData(theEnv));
}

void reclaimprocessor(void* theEnv) 
{
   shutdownprocessor(theEnv)
}
